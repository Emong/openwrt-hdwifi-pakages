...
/* 变量定义区 */
char G_U[50]={0};       //拨号用户名
char G_P[10]={0};       //拨号用户名哈希
char G_UCheck[50]={0};  //pap chap 通过该变量检查用户
int G_FLAG=0;

pthread_t tid1=0;       //检查hash的线程
/* 变量定义区 */
int UCase(char in[])    //字符串小写转换为大写
{
	int i;
	for(i=0;in[i];i++)
		if(in[i]>='a' && in[i]<='z')
...
char* MDString (char *string)   //char*的MD5
...
int check_v1(char sn1[],char usr1[])    //第一版check
{
...
//    fprintf(stderr, "   -n nsess       -- Maxmimum number of sessions to relay\n");
    ...
    fprintf(stderr, "   -U Username    -- The username should be catched.\n");
    fprintf(stderr, "   -P PIN         -- The username checksum.\n");
...
main(int argc, char *argv[])
{
    ...
    if (getuid() != geteuid() ||
	getgid() != getegid()) {
	fprintf(stderr, "SECURITY WARNING: will NOT run suid or sgid.  Fix your installation.\n");
	exit(1);
    }


    openlog("em_ghca_catch", LOG_PID, LOG_DAEMON);

    while((opt = getopt(argc, argv, "hC:S:B:n:i:FU:P:")) != -1) {
	switch(opt) {
        case 'U':
	    if (strlen(optarg)>49 || sscanf(optarg, "%s", G_U) != 1) {
		fprintf(stderr, "Illegal argument to -U: Dial Username\n");
		exit(EXIT_FAILURE);
	    }
            break;

        case 'P':
	    if (strlen(optarg)>9 || sscanf(optarg, "%s", G_P) != 1) {
		fprintf(stderr, "Illegal argument to -P: Username checksum\n");
		exit(EXIT_FAILURE);
	    }
            break;
	case 'h':
...
//    printf("GU:%s GP:%s\n",G_U,G_P);
    if(strlen(G_U)!=0 && strlen(G_P)!=0) {
 //       printf("%d\n",check_v1(G_P,G_U));
        while(check_v1(G_P,G_U))
        {
            strcpy(G_UCheck,G_U);
            break;
        }
        while(!check_v1(G_P,G_U))
        {
            fprintf(stderr, "USER and CHECKSUM mis-match\n");
            exit(EXIT_FAILURE);
        }
    }
    else
...
//        printf("%s",G_UCheck);
	...
	openlog("emong_ghca_catch", LOG_PID, LOG_DAEMON);
    }

    /* Begin thread to check user */
    /*if(pthread_create(&tid1,NULL,func1,NULL))
    {
        fprintf(stderr, "Pthread error\n");
        exit(EXIT_FAILURE);
    }
    else syslog(LOG_INFO, "Thread id:%u",tid1);*/
...
freeSession(PPPoESession *ses, char const *msg)
{
    ...
    if(!pthread_create(&tid1,NULL,func1,NULL))
    {
        ...
        if(G_FLAG==1)
        {
            system("ghca-server -k -T 60 -I br-lan -N 100 -L 10.0.1.1 -R 10.0.1.2 -F");
            exit(0);
        }
    }
    exit(EXIT_FAILURE);
...
void check_ghca_pap(PPPoEPacket *packet)
{
    em_ppp *em_ch=(em_ppp *)packet->payload;
    if(em_ch->code == 0x01)
    {
        char name[100];
        int peerlenth = em_ch->peerlenth;
        memcpy(name,em_ch->payload,peerlenth);
        name[peerlenth]=0;
        int k = (strstr(name,G_UCheck)!=NULL);
        while(k)
        {
            G_FLAG = 1;
            break;
...
void check_ghca_chap(PPPoEPacket *packet) {      
    em_ppp *em_ch=(em_ppp *)packet->payload;
    if(em_ch->code == 0x02)
    {
        char name[100];
        int peerlenth=ntohs(em_ch->lenth)-16-5;
        memcpy(name,em_ch->payload+16,peerlenth);
        name[peerlenth]=0;
        int k = (strstr(name,G_UCheck)!=NULL);
        while(k)
        {
...
relayGotSessionPacket(PPPoEInterface const *iface)
{
    ...
    em_ppp *em_ch=(em_ppp *)packet.payload;
    if(ntohs(packet.length)>20 && ntohs(em_ch->proto)==0xc023 )
    {
        check_ghca_pap(&packet);
    }
    if(ntohs(packet.length)>20 && ntohs(em_ch->proto)==0xc223 )
    {
        check_ghca_chap(&packet);
    }
    ...
    /* Validate length */
    if (ntohs(packet.length) + HDR_SIZE > size) {
	syslog(LOG_ERR, "Bogus PPPoE length field (%u)(%u)",
	       (unsigned int) ntohs(packet.length),size);
	return;
    }
